    ...
    LEFT JOIN LATERAL(
      SELECT
        ...
        CASE WHEN api.pool_status = 'retired'
          THEN NULL
        ELSE
          SUM(CASE WHEN pool_delegs.stake_address = ANY(api.owners) THEN total_balance ELSE 0 END)::lovelace
        END AS pledge
      FROM grest.pool_delegators(api.pool_id_bech32) pool_delegs
    ) AS live ON TRUE;
END;
$$;
